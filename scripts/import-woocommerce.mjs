...
async function batchUpsert(
  ...
  console.log(
    `  ✓ ${label}: ${success} lignes${failed ? ` (${failed} erreurs)` : ""}`,
  );
...
async function insertCategories(catMap) {
  ...
  const { data: tops, error: topErr } = await supabase
    .from("categories")
    .upsert(
      topLevel.map((c) => ({ name: c.name, slug: c.slug })),
      { onConflict: "slug" },
    )
    .select("id, slug");
  ...
  if (remaining.length > 0) {
    logWarn(
      "categories",
      `${remaining.length} catégories non insérées (parent introuvable)`,
    );
  }
...
async function insertProducts(parents) {
  ...
  const rows = parents.map((row) => {
    ...
    const rawSlug =
      (row["post_name"] ?? "").trim() || slugify(row["post_title"] ?? "");
    const sku = row["sku"].trim();
    ...
    const pbqType = pbqEnabled
      ? (nullable(row["meta:pbq_pricing_type"]) ?? "fixed")
      : null;

    // post_content : ignorer si c'est du JSON Elementor
    const rawContent = (row["post_content"] ?? "").trim();
    const description = isElementorJson(rawContent)
      ? null
      : nullable(rawContent);
    ...
    return {
      ...
      backorders_allowed: ["yes", "notify"].includes(
        (row["backorders"] ?? "").trim(),
      ),
...
async function insertVariants(parents, children, skuToId) {
  ...
  for (const row of children) {
    ...
    if (!productId) {
      logWarn(
        "variants",
        `Parent SKU introuvable: "${parentSku}" pour variation "${row["sku"]}"`,
      );
      continue;
    }
    ...
    variantRows.push({
      ...
      height: toFloat(row["height"]),
      min_order_quantity:
        toInt(row["meta:variation_minimum_allowed_quantity"]) ?? 1,
      status: "publish",
...
async function insertVariantAttributes(
  children,
  varSkuToId,
  attrSlugToId,
  headers,
  ...
  const metaAttrCols = headers.filter((h) =>
    h.startsWith("meta:attribute_pa_"),
  );
  const vaRows = [];
...
async function insertPriceTiers(parents, children, skuToId, varSkuToId) {
  ...
  for (const row of parents) {
    const pbqEnabled =
      (row["meta:pbq_pricing_type_enable"] ?? "").trim() === "enable";
    ...
    const pricingType =
      (row["meta:pbq_pricing_type"] ?? "fixed").trim() || "fixed";
    const tiers = parsePBQTiers(
      row["meta:pbq_discount_table_data"] ?? "",
      pricingType,
    );
  ...
  const parentPricingType = new Map(
    parents.map((r) => [
      r["sku"].trim(),
      (r["meta:pbq_pricing_type"] ?? "fixed").trim() || "fixed",
    ]),
  );
...
async function main() {
  ...
  if (pingErr) {
    ...
    console.error(
      "   → Fournissez la clé service_role : SUPABASE_KEY=xxx node scripts/import-woocommerce.mjs",
    );
    process.exit(1);
  }
