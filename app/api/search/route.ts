...
export async function GET(request: NextRequest) {
  ...
  if (!rateLimit(ip, 30, 60000)) {
    return NextResponse.json(
      { results: [], error: "Trop de requêtes" },
      { status: 429 },
    );
  }
  ...
  let q1 = supabase
    .from("products")
    .select(
      "id, name, slug, sku, regular_price, product_images!inner(url, is_featured, position)",
    )
    .or(`name.ilike.%${q}%,sku.ilike.%${q}%`)
    .eq("status", "publish");
  if (childIds.size > 0)
    q1 = q1.not("id", "in", `(${[...childIds].join(",")})`);
  ...
  const results = (data ?? []).map((p: any) => {
    ...
    const featured =
      images.find((i: any) => i.is_featured) ??
