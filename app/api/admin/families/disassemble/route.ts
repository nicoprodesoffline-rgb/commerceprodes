...
export async function POST(req: NextRequest) {
  ...
  if (Array.isArray(body.member_ids) && body.member_ids.length > 0) {
    const ids = (body.member_ids as unknown[]).filter(
    ...
    (removedMembers || []).forEach((m: any) => {
      if (m.member_product_id)
        removedProductIds.add(String(m.member_product_id));
    });
    ...
    log("info", "admin.family.disassemble_members", {
      family_id,
      count: ids.length,
    });
  ...
  (allMembers || []).forEach((m: any) => {
    if (m.member_product_id)
      detachedProductIds.add(String(m.member_product_id));
  });
  ...
  log("info", "admin.family.disassemble", { family_id });
