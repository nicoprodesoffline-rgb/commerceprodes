...
export async function GET(req: NextRequest) {
  ...
  const rows: string[] = [
    ["family_id", "family_name", "strategy", "active", "parent_sku", "parent_name", "member_type", "member_sku", "member_name"].join(",")
  ];

  for (const f of data ?? []) {
    const parent = (f.products as unknown) as { name: string; sku: string } | null;
    const members = ((f.product_family_members as unknown) as Array<{
      active: boolean; member_type: string;
      products: { name: string; sku: string } | null;
      variants: { name: string; sku: string } | null;
    }>) ?? [];

    if (members.length === 0) {
      rows.push([f.id, f.name, f.strategy, f.active, parent?.sku ?? "", parent?.name ?? "", "", "", ""].map(escapeCsv).join(","));
    } else {
      for (const m of members) {
        if (!m.active) continue;
        const mp = m.products ?? m.variants;
        rows.push([f.id, f.name, f.strategy, f.active, parent?.sku ?? "", parent?.name ?? "", m.member_type, mp?.sku ?? "", mp?.name ?? ""].map(escapeCsv).join(","));
      }
    }
  }
