...
export async function PATCH(
  ...
  const ordered = (body.ordered_ids ?? []).filter((x) => typeof x === "string" && UUID_RE.test(x));
  if (ordered.length === 0) return NextResponse.json({ error: "ordered_ids requis" }, { status: 400 });
  ...
  console.log(JSON.stringify({ event: "admin.family.members.reorder", family_id: id, count: ordered.length }));
