...
type TabId = "imports" | "normalisation" | "families" | "pricing" | "validation";
...
const TABS: Array<{ id: TabId; label: string; hint: string }> = [
  { id: "imports", label: "Import brut", hint: "Upload catalogue + déclenchement pipeline" },
  { id: "normalisation", label: "Normalisation", hint: "Règles explicites, IA, coût, plan d'action" },
  { id: "families", label: "Mères / Filles", hint: "Arbre mère/filles + audit/suggestions" },
  { id: "pricing", label: "Tarification", hint: "Vue macro pricing normal/promo" },
  { id: "validation", label: "Validation & publication", hint: "Queue des points à vérifier" },
];
...
export default function DataFactoryPage() {
  ...
  const [catalogYear, setCatalogYear] = useState(String(new Date().getFullYear()));
  ...
  const [puidDryRunResult, setPuidDryRunResult] = useState<null | {
    dry_run: true;
    scope: { mode: string; since: string | null; total_products: number; total_variants: number; total_suggestions: number; collisions: number; lot_candidates: number };
    preview: { products: PuidSuggestion[]; variants: PuidSuggestion[]; branches: unknown[]; collisions: unknown[] };
    apply: { products_planned: number; variants_planned: number; product_sku_conflicts: unknown[]; variant_sku_conflicts: unknown[] };
  }>(null);
  ...
  const [puidAssemblePlan, setPuidAssemblePlan] = useState<null | {
    dry_run: boolean;
    stats: { total_products_with_puid_root: number; unique_roots: number; to_create: number; skip_existing: number; skip_single: number };
    plan?: unknown[];
  }>(null);
  ...
  async function loadImports() {
    ...
    try {
      ...
      const res = await adminFetch(`/api/admin/import-logs?${params.toString()}`);
      const data = (await res.json().catch(() => ({}))) as { logs?: ImportLog[]; error?: string };
      if (!res.ok) throw new Error(data.error ?? "Impossible de charger les imports");
  ...
  async function submitImport() {
    ...
    try {
      ...
      const statusBits = [
        ...
        data.webhook_started ? "workflow n8n lancé" : data.webhook_message ?? "workflow non lancé",
  ...
  async function loadConfig() {
    ...
    try {
      ...
      const data = (await res.json().catch(() => ({}))) as { config?: Record<string, string>; error?: string };
      if (!res.ok) throw new Error(data.error ?? "Impossible de charger la configuration");
  ...
  async function loadFamilyAudit() {
    ...
    try {
      ...
      const res = await adminFetch(`/api/admin/families/audit${params.size ? `?${params.toString()}` : ""}`);
      const data = (await res.json().catch(() => ({}))) as FamilyAudit & { error?: string };
      if (!res.ok) throw new Error(data.error ?? "Impossible de charger l'audit familles");
      setFamilyAudit(data);
    } catch (err) {
  ...
  async function loadProducts() {
    ...
    try {
      ...
      const res = await adminFetch(`/api/admin/products-list?${params.toString()}`);
      const data = (await res.json().catch(() => ({}))) as { products?: ProductListItem[]; error?: string };
  ...
  useEffect(() => {
    void Promise.all([loadImports(), loadConfig(), loadFamilyAudit(), loadProducts()]);
  }, []);
  ...
  async function runSuggestionPreview() {
    ...
    try {
      ...
      if (!res.ok) throw new Error(data.error ?? "Échec simulation normalisation");
      setSuggestions(data.suggestions ?? []);
      setMessage(`Simulation terminée: ${(data.suggestions ?? []).length} suggestions`);
    } catch (err) {
  ...
  async function runPuidPreview() {
    ...
    try {
      ...
      const res = await adminFetch(`/api/admin/data-factory/puid?${params.toString()}`);
      const data = (await res.json().catch(() => ({}))) as PuidPreview & { error?: string };
  ...
  async function applyPuid() {
    ...
    try {
      ...
      if (!res.ok) {
        const hint = data.migration ? ` · migration requise: ${data.migration}` : "";
  ...
  async function runPuidDryRun() {
    ...
    try {
      ...
      const data = (await res.json().catch(() => ({}))) as typeof puidDryRunResult & { error?: string };
      if (!res.ok) throw new Error(data?.error ?? "Échec dry-run PUID");
      setPuidDryRunResult(data as typeof puidDryRunResult);
      setMessage(`Dry-run PUID: ${data?.apply?.products_planned ?? 0} produits + ${data?.apply?.variants_planned ?? 0} variantes à modifier`);
    } catch (err) {
  ...
  async function downloadBackup() {
    ...
    setError(null);
    try {
      const res = await adminFetch(`/api/admin/data-factory/puid/backup?limit=5000`);
      const data = await res.json().catch(() => ({})) as { error?: string };
  ...
  async function restoreFromFile(file: File) {
    ...
    try {
      ...
      const parsed = JSON.parse(text) as { products?: unknown[]; variants?: unknown[] };
      if (!Array.isArray(parsed.products) || !Array.isArray(parsed.variants)) {
        throw new Error("Fichier backup invalide (products/variants manquants)");
      }
      ...
      const data = (await res.json().catch(() => ({}))) as {
        restored_products?: number; restored_variants?: number; errors?: string[]; error?: string;
      };
      if (!res.ok) throw new Error(data.error ?? "Échec restauration PUID");
      setMessage(`Rollback PUID appliqué: ${data.restored_products ?? 0} produits + ${data.restored_variants ?? 0} variantes restaurés`);
  ...
  async function runAssembleFamilies(applyNow: boolean) {
    ...
    setError(null);
    try {
      const res = await adminFetch("/api/admin/data-factory/puid/assemble-families", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          dry_run: !applyNow,
      ...
      const data = (await res.json().catch(() => ({}))) as typeof puidAssemblePlan & { error?: string };
      if (!res.ok) throw new Error(data?.error ?? "Échec assemblage familles");
      setPuidAssemblePlan(data as typeof puidAssemblePlan);
      if (applyNow) {
        setMessage(`Familles assemblées depuis PUID: ${(data as { created_families?: number }).created_families ?? 0} nouvelles familles créées`);
        await loadFamilyAudit();
      } else {
        setMessage(`Dry-run assemblage: ${data?.stats?.to_create ?? 0} familles à créer`);
      }
    } catch (err) {
      setError(err instanceof Error ? err.message : "Erreur assemblage familles");
    } finally {
  ...
  const pricingSummary = useMemo<PricingSummary>(() => {
    return products.reduce<PricingSummary>(
      (acc, p) => {
        ...
        if (typeof p.sale_price === "number" && p.sale_price > 0) acc.promoActive += 1;
        if (p.pbq_enabled) acc.pbqEnabled += 1;
        if (p.pbq_pricing_type === "degressive") acc.degressive += 1;
        if (p.pbq_pricing_type === "lot") acc.lot += 1;
        if (p.regular_price == null || p.regular_price <= 0) acc.missingPrice += 1;
        return acc;
      },
      {
        total: 0,
  ...
  const validationQueue = useMemo<ValidationIssue[]>(() => {
    ...
    return products
      .map((product) => {
        ...
        if (product.status === "publish" && (!product.regular_price || product.regular_price <= 0)) {
          reasons.push("Prix publié invalide");
        }
        if (!product.short_description || product.short_description.trim().length < 20) {
  ...
  const inputTokensPerItem = parseNumber(cfg("df_cost_input_tokens_per_item", "900"), 900);
  const outputTokensPerItem = parseNumber(cfg("df_cost_output_tokens_per_item", "300"), 300);
  const inputPerMillion = parseNumber(cfg("df_cost_input_per_million", "1"), 1);
  const outputPerMillion = parseNumber(cfg("df_cost_output_per_million", "5"), 5);

  const estimatedCost = useMemo(() => {
    const inputCost = (batchSize * inputTokensPerItem * inputPerMillion) / 1_000_000;
    const outputCost = (batchSize * outputTokensPerItem * outputPerMillion) / 1_000_000;
    const total = inputCost + outputCost;
    return { inputCost, outputCost, total };
  }, [batchSize, inputTokensPerItem, outputTokensPerItem, inputPerMillion, outputPerMillion]);
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">
            Pipeline catalogue: import brut → normalisation → mères/filles → validation.
          </p>
        </div>
        <div className="flex gap-2">
          ...
          <button
            onClick={() => void Promise.all([loadImports(), loadConfig(), loadFamilyAudit(), loadProducts()])}
          ...
          {activeTab === "imports" && (
            <section className="space-y-4">
              <div className="rounded-lg border border-blue-200 bg-blue-50 p-4">
                <h2 className="text-sm font-semibold text-blue-900">Processus d&apos;import catalogue</h2>
                <ol className="mt-2 grid gap-2 text-xs text-blue-900 md:grid-cols-4">
                  <li className="rounded border border-blue-200 bg-white/70 p-2">1. Upload fichier brut (Excel/CSV)</li>
                  <li className="rounded border border-blue-200 bg-white/70 p-2">2. Parsing & extraction (Retab / n8n)</li>
                  <li className="rounded border border-blue-200 bg-white/70 p-2">3. Normalisation PUID + Mère/Filles</li>
                  <li className="rounded border border-blue-200 bg-white/70 p-2">4. Validation humaine avant publication</li>
                </ol>
              </div>

              <div className="rounded-lg border border-gray-200 p-4">
                <h2 className="mb-3 text-sm font-semibold text-gray-900">Nouveau catalogue fournisseur</h2>
                ...
                <div className="mt-3 flex flex-wrap items-center gap-2">
                  ...
                  {importFile ? (
                    <span className="rounded-full bg-gray-100 px-2 py-1 text-xs text-gray-700">
                      {importFile.name} · {(importFile.size / 1024).toFixed(0)} Ko
                    </span>
                  ) : (
                    <span className="text-xs text-gray-500">Formats acceptés: .xlsx .xls .csv .ods</span>
                  )}
                  <button
              ...
              <div className="grid gap-3 sm:grid-cols-4">
                <div className="rounded-lg border border-gray-200 bg-gray-50 p-3">
                  ...
                  <p className="text-2xl font-semibold text-gray-900">{imports.length}</p>
                </div>
                <div className="rounded-lg border border-gray-200 bg-gray-50 p-3">
                ...
                <div className="rounded-lg border border-gray-200 bg-gray-50 p-3">
                  <p className="text-xs text-gray-500">Terminés / exploitables</p>
                  <p className="text-2xl font-semibold text-gray-900">
                  ...
                  <table className="w-full text-sm">
                    <thead className="bg-gray-50 text-left text-xs text-gray-500">
                      <tr>
                        ...
                        <th className="px-3 py-2 text-right">Lignes (ok/err)</th>
                    ...
                    <tbody className="divide-y divide-gray-100">
                      {imports.map((item) => (
                        <tr key={item.id}>
                          <td className="px-3 py-2 text-gray-500">{fmtDate(item.created_at)}</td>
                          ...
                          <td className="px-3 py-2 font-medium text-gray-800">
                            {item.file_url ? (
                              <a href={item.file_url} target="_blank" className="hover:underline" rel="noreferrer">
                                {item.filename ?? "—"}
                              </a>
                            ) : (
                              item.filename ?? "—"
                          ...
                          <td className="px-3 py-2">
                            <span
                              className={`rounded-full px-2 py-0.5 text-xs font-medium ${
                                STATUS_META[item.status] ?? "bg-gray-100 text-gray-700"
                              }`}
                            >
                              {item.status}
                            </span>
                          ...
                          <td className="px-3 py-2 text-xs text-gray-500">{item.import_batch_key ?? "—"}</td>
                          <td className="px-3 py-2 text-xs text-gray-500">{item.stage ?? "—"}</td>
          ...
          {activeTab === "normalisation" && (
            <section className="space-y-5">
              <div className="rounded-lg border border-amber-200 bg-amber-50 p-4">
                <h2 className="text-sm font-semibold text-amber-900">Ce que fait la normalisation (concrètement)</h2>
                <ul className="mt-2 space-y-1 text-xs text-amber-900">
                  <li>1. Détection des groupes candidats (via parent_sku, racine SKU, titre).</li>
                  <li>2. Séparation attributs qui changent le prix vs attributs esthétiques.</li>
                  <li>3. Proposition de structure Mère/Filles + besoin de validation humaine si doute.</li>
                  <li>4. Génération d&apos;un plan de publication (brouillon / publié sans prix / publié avec prix).</li>
                </ul>
              </div>

              <div className="rounded-lg border border-gray-200 p-4">
                <h2 className="mb-3 text-sm font-semibold text-gray-900">Règles de normalisation</h2>
                <div className="grid gap-3 md:grid-cols-2">
                  <label className="space-y-1 text-sm">
                    <span className="text-gray-600">Stratégie de groupement</span>
                    <select
                      value={normalizationStrategy}
                      onChange={(e) =>
                        setConfig((prev) => ({ ...prev, df_normalization_strategy: e.target.value }))
                      }
                      className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm"
                    >
                  ...
                  <label className="space-y-1 text-sm">
                    ...
                    <select
                      value={publishPolicy}
                      onChange={(e) =>
                        setConfig((prev) => ({ ...prev, df_publish_policy: e.target.value }))
                      }
                      className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm"
                    >
                      <option value="draft">Brouillon</option>
                      <option value="publish_if_price">Publier si prix valide</option>
                      <option value="publish_without_price">Publier même sans prix</option>
                    </select>
                  </label>
                  <label className="space-y-1 text-sm">
                    <span className="text-gray-600">Seuil confiance (0-100)</span>
                    <input
                      type="number"
                      min={0}
                      max={100}
                      ...
                      onChange={(e) =>
                        setConfig((prev) => ({ ...prev, df_confidence_threshold: e.target.value }))
                      }
                      className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm"
                    />
                  </label>
                  <label className="space-y-1 text-sm">
                    <span className="text-gray-600">Séparateur SKU prix/non-prix</span>
                    <input
                      type="text"
                      value={cfg("df_sku_separator", ".")}
                      onChange={(e) => setConfig((prev) => ({ ...prev, df_sku_separator: e.target.value }))}
                      className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm"
                    />
                  </label>
                  <label className="space-y-1 text-sm md:col-span-2">
                    <span className="text-gray-600">Axes prix-impact (liste comma-separated)</span>
                    <input
                      type="text"
                      value={cfg("df_price_impact_axes_default", "dimension,norme,finition")}
                      onChange={(e) =>
                        setConfig((prev) => ({ ...prev, df_price_impact_axes_default: e.target.value }))
                      }
                      className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm"
                    />
                  </label>
                ...
                <div className="mt-4 flex flex-wrap gap-2">
                  ...
                  >
                    {suggestionsLoading ? "Simulation..." : "Simuler regroupement"}
                  </button>
                </div>
                <div className="mt-3 rounded border border-gray-100 bg-gray-50 p-3 text-xs text-gray-600">
                  <p><strong>Stratégie de groupement:</strong> <code>parent_sku</code> est la plus stable sur vos données legacy.</p>
                  <p><strong>Politique publication:</strong> utilisez <code>publish_without_price</code> pour CTA &quot;demande de prix&quot;.</p>
                  <p><strong>Seuil confiance:</strong> en dessous du seuil, l&apos;item est routé vers la queue <code>need_check</code>.</p>
                </div>
              </div>

              <div className="rounded-lg border border-gray-200 p-4">
                <h2 className="mb-3 text-sm font-semibold text-gray-900">Centre de contrôle IA</h2>
                <div className="grid gap-3 md:grid-cols-2">
                  <label className="space-y-1 text-sm">
                    <span className="text-gray-600">Modèle extraction</span>
                    <input
                      type="text"
                      value={cfg("df_ai_model_extract", "claude-haiku-4-5-20251001")}
                      onChange={(e) => setConfig((prev) => ({ ...prev, df_ai_model_extract: e.target.value }))}
                      className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm"
                    />
                  </label>
                  <label className="space-y-1 text-sm">
                    ...
                    <input
                      ...
                      onChange={(e) => setConfig((prev) => ({ ...prev, df_ai_model_normalize: e.target.value }))}
                      className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm"
                    />
                  </label>
                  <label className="space-y-1 text-sm md:col-span-2">
                    ...
                    <textarea
                      rows={3}
                      value={cfg("df_ai_prompt_extract", "Extraire SKU, désignation, prix, logistique, options, taxes.")}
                      onChange={(e) => setConfig((prev) => ({ ...prev, df_ai_prompt_extract: e.target.value }))}
                      className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm"
                    />
                  </label>
                  <label className="space-y-1 text-sm md:col-span-2">
                    ...
                    <textarea
                      ...
                      onChange={(e) => setConfig((prev) => ({ ...prev, df_ai_prompt_normalize: e.target.value }))}
                      className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm"
                    />
                  </label>
                </div>
                ...
                <div className="mt-4 grid gap-3 md:grid-cols-4">
                  <label className="space-y-1 text-sm">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setConfig((prev) => ({ ...prev, df_cost_input_tokens_per_item: e.target.value }))
                      }
                      className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm"
                    />
                  </label>
                  <label className="space-y-1 text-sm">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setConfig((prev) => ({ ...prev, df_cost_output_tokens_per_item: e.target.value }))
                      }
                      className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm"
                    />
                  </label>
                  <label className="space-y-1 text-sm">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setConfig((prev) => ({ ...prev, df_cost_input_per_million: e.target.value }))
                      }
                      className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm"
                    />
                  </label>
                  <label className="space-y-1 text-sm">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setConfig((prev) => ({ ...prev, df_cost_output_per_million: e.target.value }))
                      }
                      className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm"
                    />
                  </label>
                ...
                <div className="mt-4 rounded-lg border border-blue-200 bg-blue-50 p-3">
                  <div className="mb-2 flex items-center justify-between gap-3">
                    <p className="text-sm font-medium text-blue-900">Calculer le prix d&apos;un batch IA</p>
                    <input
                      type="number"
                      min={1}
                      value={batchSize}
                  ...
                  <div className="grid gap-2 text-sm text-blue-900 md:grid-cols-3">
                    <p>Input estimé: <strong>{fmtMoney(estimatedCost.inputCost)}</strong></p>
                    <p>Output estimé: <strong>{fmtMoney(estimatedCost.outputCost)}</strong></p>
                    <p>Total estimé: <strong>{fmtMoney(estimatedCost.total)}</strong></p>
                  </div>
                </div>
              ...
              <div className="rounded-lg border border-gray-200 p-4">
                <h2 className="mb-2 text-sm font-semibold text-gray-900">Prévisualisation suggestions</h2>
                {suggestions.length === 0 ? (
                  <p className="text-sm text-gray-500">
                    Lance une simulation pour afficher les regroupements proposés.
                  </p>
                ) : (
                  <div className="space-y-2">
                    {suggestions.slice(0, 8).map((suggestion, index) => (
                      <div key={`${suggestion.parent.id}-${index}`} className="rounded-md border border-gray-200 p-3">
                        <p className="text-sm font-medium text-gray-900">
                          {suggestion.parent.name} ({suggestion.children.length} enfants)
                        </p>
                        <p className="mt-1 text-xs text-gray-500">
                          Strategy: {suggestion.strategy} · Score: {suggestion.score.toFixed(2)}
                        </p>
                      </div>
                    ))}
                  </div>
              ...
              <div className="rounded-lg border border-indigo-200 p-4">
                <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
                  <div>
                    <h2 className="text-sm font-semibold text-indigo-900">Générateur PUID — Workflow en 4 étapes</h2>
                    <p className="text-xs text-indigo-700">
                      1) Prévisualiser · 2) Dry-run · 3) Backup · 4) Appliquer / Rollback
                    </p>
                  </div>
                  <div className="flex items-center gap-2 text-xs">
                    ...
                    <label className="flex items-center gap-1 text-gray-600">
                      ...
                      <input
                        ...
                        max={3000}
                        value={puidLimit}
                        onChange={(e) => setPuidLimit(Math.max(20, Math.min(3000, Number(e.target.value) || 20)))}
                ...
                <div className="mb-3 flex flex-wrap gap-2 text-xs">
                  <label className="flex items-center gap-1 rounded border border-gray-200 bg-gray-50 px-2 py-1 text-gray-700">
                    <input type="checkbox" checked={puidApplyToSku} onChange={(e) => setPuidApplyToSku(e.target.checked)} />
                    Remplacer aussi les SKU
                  </label>
                  <label className="flex items-center gap-1 rounded border border-gray-200 bg-gray-50 px-2 py-1 text-gray-700">
                    <input type="checkbox" checked={puidCleanupLots} onChange={(e) => setPuidCleanupLots(e.target.checked)} />
                ...
                <div className="mb-3 flex flex-wrap gap-2">
                  ...
                  >
                    {puidDryRunLoading ? "Dry-run..." : "② Dry-run (simuler apply)"}
                  </button>
                </div>
                ...
                {puidDryRunResult && (
                  <div className="mb-3 rounded border border-blue-200 bg-blue-50 p-3 text-xs">
                    <p className="font-semibold text-blue-900 mb-1">Résultat dry-run :</p>
                    <div className="grid grid-cols-2 gap-1 text-blue-800 sm:grid-cols-4">
                      <span>Produits à modifier: <strong>{puidDryRunResult.apply?.products_planned ?? 0}</strong></span>
                      <span>Variantes à modifier: <strong>{puidDryRunResult.apply?.variants_planned ?? 0}</strong></span>
                      <span>Conflits SKU produits: <strong>{(puidDryRunResult.apply?.product_sku_conflicts as unknown[])?.length ?? 0}</strong></span>
                      <span>Conflits SKU variantes: <strong>{(puidDryRunResult.apply?.variant_sku_conflicts as unknown[])?.length ?? 0}</strong></span>
                    </div>
                  </div>
                )}
                ...
                <div className="mb-3 flex flex-wrap gap-2">
                  ...
                  >
                    {puidBackupLoading ? "Sauvegarde..." : "③ Backup (télécharger état actuel)"}
                  </button>
                  <button
                  ...
                  >
                    {puidApplyLoading ? "Application..." : "④ Appliquer PUID (global)"}
                  </button>
                </div>

                {/* Rollback */}
                <div className="mb-4 rounded border border-red-100 bg-red-50 p-3">
                  <p className="mb-2 text-xs font-semibold text-red-800">Rollback — Restaurer depuis backup</p>
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    />
                    {puidRestoreLoading && (
                      <span className="text-xs text-red-700">Restauration en cours...</span>
                    )}
                  </div>
                  <p className="mt-1 text-[11px] text-red-600">
                    Sélectionner un fichier backup JSON pour restaurer les PUID/SKU précédents.
                  </p>
                </div>

                {/* Assemblage familles depuis PUID */}
                <div className="rounded border border-green-200 bg-green-50 p-3">
                  <p className="mb-2 text-xs font-semibold text-green-900">Assemblage auto mère/filles depuis PUID root</p>
                  <p className="mb-2 text-[11px] text-green-700">
                    Regroupe les produits partageant le même puid_root en familles (mère + filles).
                  </p>
                  <div className="flex gap-2">
                    <button
                    ...
                    >
                      {puidAssembleLoading ? "Analyse..." : "Simuler assemblage"}
                    </button>
                    <button
                    ...
                    >
                      {puidAssembleLoading ? "Assemblage..." : "Appliquer assemblage"}
                    </button>
                  </div>
                  {puidAssemblePlan && (
                    <div className="mt-2 grid grid-cols-3 gap-2 text-[11px] text-green-800 sm:grid-cols-5">
                      <span>Produits PUID: <strong>{puidAssemblePlan.stats?.total_products_with_puid_root ?? 0}</strong></span>
                      <span>Racines uniques: <strong>{puidAssemblePlan.stats?.unique_roots ?? 0}</strong></span>
                      <span>Familles à créer: <strong>{puidAssemblePlan.stats?.to_create ?? 0}</strong></span>
                      <span>Déjà existantes: <strong>{puidAssemblePlan.stats?.skip_existing ?? 0}</strong></span>
                      <span>Ignorées (1 seul): <strong>{puidAssemblePlan.stats?.skip_single ?? 0}</strong></span>
                    </div>
                  )}
                </div>
                ...
                {!puidPreview ? (
                  <p className="mt-3 text-xs text-gray-500">
                    Lancez une prévisualisation pour vérifier la structure racine → embranchements.
                  </p>
                ) : (
                  <div className="mt-4 space-y-3">
                    <div className="grid gap-3 sm:grid-cols-5">
                      <div className="rounded border border-gray-200 bg-gray-50 p-2">
                        <p className="text-[11px] text-gray-500">Produits analysés</p>
                        <p className="text-lg font-semibold text-gray-900">{puidPreview.scope.total_products}</p>
                      </div>
                      <div className="rounded border border-gray-200 bg-gray-50 p-2">
                        <p className="text-[11px] text-gray-500">Variantes analysées</p>
                        <p className="text-lg font-semibold text-gray-900">{puidPreview.scope.total_variants}</p>
                      </div>
                      <div className="rounded border border-gray-200 bg-gray-50 p-2">
                        <p className="text-[11px] text-gray-500">Suggestions</p>
                        <p className="text-lg font-semibold text-gray-900">{puidPreview.scope.total_suggestions}</p>
                      </div>
                      <div className="rounded border border-gray-200 bg-gray-50 p-2">
                        <p className="text-[11px] text-gray-500">Collisions</p>
                        <p className="text-lg font-semibold text-gray-900">{puidPreview.scope.collisions}</p>
                      </div>
                      <div className="rounded border border-amber-200 bg-amber-50 p-2">
                        <p className="text-[11px] text-amber-700">Lignes lot détectées</p>
                        <p className="text-lg font-semibold text-amber-800">{puidPreview.scope.lot_candidates}</p>
                      </div>
                    </div>

                    <div className="rounded border border-gray-200">
                      <div className="border-b border-gray-100 bg-gray-50 px-3 py-2 text-xs font-medium text-gray-700">
                        Exemples variantes (racine / branche prix / branche style)
                      </div>
                        ...
                        <table className="w-full text-xs">
                          ...
                          <tbody className="divide-y divide-gray-100">
                            {puidPreview.variants.slice(0, 20).map((row) => (
                              <tr key={row.id}>
                                <td className="px-3 py-2 font-mono text-indigo-700">{row.suggested_puid}</td>
                                <td className="px-3 py-2 text-gray-500">{row.source_sku ?? "—"}</td>
                                <td className="px-3 py-2 text-gray-700">{row.price_branch ?? "BASE"}</td>
                                <td className="px-3 py-2 text-gray-500">{row.style_branch ?? "—"}</td>
                                <td className="px-3 py-2 text-gray-600">{Math.round(row.confidence * 100)}%</td>
                              </tr>
                            ))}
                          </tbody>
                        </table>
            ...
            <section className="space-y-4">
              <div className="flex gap-2">
                <button
                  ...
                  className="rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50 disabled:opacity-60"
                >
                  {familyAuditLoading ? "Analyse..." : "Relancer audit mères/filles"}
                </button>
                <Link
                  href="/admin/familles"
                ...
                <>
                  ...
                  <div className="grid gap-3 sm:grid-cols-4">
                    <div className="rounded-lg border border-gray-200 bg-gray-50 p-3">
                      ...
                      <p className="text-2xl font-semibold text-gray-900">{familyAudit.total_active_families}</p>
                    </div>
                    <div className="rounded-lg border border-gray-200 bg-gray-50 p-3">
                      <p className="text-xs text-gray-500">Candidats en attente</p>
                      <p className="text-2xl font-semibold text-gray-900">{familyAudit.pending_candidates}</p>
                    </div>
                    <div className="rounded-lg border border-gray-200 bg-gray-50 p-3">
                      <p className="text-xs text-gray-500">Suggestions potentielles</p>
                      <p className="text-2xl font-semibold text-gray-900">{familyAudit.potential_suggestions ?? 0}</p>
                    </div>
                    <div className="rounded-lg border border-gray-200 bg-gray-50 p-3">
                      <p className="text-xs text-gray-500">Issues audit</p>
                      <p className="text-2xl font-semibold text-gray-900">{familyAudit.issues}</p>
                    </div>
                  </div>

                  <div className="grid gap-4 lg:grid-cols-3">
                    <div className="rounded-lg border border-gray-200 p-3">
                      <p className="mb-2 text-sm font-semibold text-gray-800">Parents sans enfants</p>
                      {familyAudit.parents_without_children.length === 0 ? (
                        <p className="text-xs text-gray-500">Aucun.</p>
                      ) : (
                        <ul className="space-y-1 text-xs text-gray-700">
                          {familyAudit.parents_without_children.slice(0, 8).map((row) => (
                    ...
                    <div className="rounded-lg border border-gray-200 p-3">
                      <p className="mb-2 text-sm font-semibold text-gray-800">Orphelins parent_sku</p>
                      {familyAudit.orphan_products.length === 0 ? (
                        <p className="text-xs text-gray-500">Aucun.</p>
                      ) : (
                        <ul className="space-y-1 text-xs text-gray-700">
                          {familyAudit.orphan_products.slice(0, 8).map((row) => (
                            <li key={row.id}>
                              {row.name} <span className="text-gray-400">({row.parent_sku})</span>
                            </li>
                          ))}
                        </ul>
                      )}
                    ...
                    <div className="rounded-lg border border-gray-200 p-3">
                      <p className="mb-2 text-sm font-semibold text-gray-800">Familles volumineuses</p>
            ...
            <section className="space-y-4">
              <div className="grid gap-3 sm:grid-cols-3 lg:grid-cols-6">
                <div className="rounded-lg border border-gray-200 bg-gray-50 p-3">
                  ...
                  <p className="text-2xl font-semibold text-gray-900">{pricingSummary.total}</p>
                </div>
                <div className="rounded-lg border border-gray-200 bg-gray-50 p-3">
                  <p className="text-xs text-gray-500">Publiés</p>
                  <p className="text-2xl font-semibold text-gray-900">{pricingSummary.published}</p>
                </div>
                <div className="rounded-lg border border-gray-200 bg-gray-50 p-3">
                  <p className="text-xs text-gray-500">Promo active</p>
                  <p className="text-2xl font-semibold text-gray-900">{pricingSummary.promoActive}</p>
                </div>
                <div className="rounded-lg border border-gray-200 bg-gray-50 p-3">
                  <p className="text-xs text-gray-500">PBQ activé</p>
                  <p className="text-2xl font-semibold text-gray-900">{pricingSummary.pbqEnabled}</p>
                </div>
                <div className="rounded-lg border border-gray-200 bg-gray-50 p-3">
                  <p className="text-xs text-gray-500">Dégressif</p>
                  <p className="text-2xl font-semibold text-gray-900">{pricingSummary.degressive}</p>
                </div>
                <div className="rounded-lg border border-gray-200 bg-gray-50 p-3">
                  <p className="text-xs text-gray-500">Prix manquant</p>
                  <p className="text-2xl font-semibold text-gray-900">{pricingSummary.missingPrice}</p>
                </div>
              </div>
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {products.slice(0, 20).map((product) => (
                      <tr key={product.id}>
                        ...
                        <td className="px-3 py-2 text-right text-gray-700">
                          {typeof product.regular_price === "number" ? fmtMoney(product.regular_price) : "—"}
                        </td>
                        <td className="px-3 py-2 text-gray-600">{product.pbq_pricing_type ?? "flat"}</td>
                        <td className="px-3 py-2 text-gray-500">{product.updated_at ? fmtDate(product.updated_at) : "—"}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
                      ...
                      {validationQueue.map((entry) => (
                        <tr key={entry.product.id}>
                          <td className="px-3 py-2 text-gray-800">
                            <Link href={`/admin/produits/${entry.product.id}`} className="hover:underline">
                              {entry.product.name}
                            </Link>
                          </td>
                          <td className="px-3 py-2 text-gray-500">{entry.product.sku ?? "—"}</td>
                          <td className="px-3 py-2 text-gray-600">{entry.product.status}</td>
                          <td className="px-3 py-2 text-xs text-red-700">{entry.reasons.join(" · ")}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
