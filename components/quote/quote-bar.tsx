  ...
  if (quoteCount === 0) {
    return (
      <button
        onClick={() =>
          toast.info(
            "Ajoutez des produits à votre devis depuis les fiches produits.",
          )
        }
          ...
          <div className="relative z-10 w-full max-w-lg rounded-t-2xl bg-white shadow-xl sm:rounded-2xl">
            ...
            <div className="flex items-center justify-between border-b border-gray-200 px-5 py-4">
              <h2 className="font-semibold text-gray-900">
                Devis groupé — {quoteItems.length} produit
...
function QuoteSendButton({
  items,
  onClose,
}: {
  items: { title: string; sku: string | null; quantity: number }[];
  onClose: () => void;
}) {
  const refs = items
